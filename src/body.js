...
export function extractAiBody(aiText, pattern = '') {
    ...
    if (!rule) {
        return { text: stripHtmlComments(fullText), mode: 'full', matched: true, error: '' };
    }

    try {
        const match = new RegExp(rule, 'u').exec(fullText);
        if (!match) {
            return { text: stripHtmlComments(fullText), mode: 'fallback', matched: false, error: 'no_match' };
        }
        const captured = match.length > 1 ? match[1] : match[0];
        if (!String(captured ?? '').trim()) {
            return { text: stripHtmlComments(fullText), mode: 'fallback', matched: false, error: 'empty_match' };
        }
        return { text: stripHtmlComments(captured), mode: 'regex', matched: true, error: '' };
    } catch (error) {
        return {
            text: stripHtmlComments(fullText),
            mode: 'fallback',
            matched: false,

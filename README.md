...
安装后，酒馆顶部会出现记忆中心按钮。完整的记忆、章节、待审、后台任务和设置都在独立 drawer 中；Extensions 菜单与扩展设置页也保留快捷入口。
...
设置页提供「保存并测试连接」，只返回脱敏后的连接状态，不显示 API Key 或供应商原始响应。
...
- **延迟提取**：上一楼在下一楼用户发言时（或切聊 / 生成前入队）才提取；单纯打开界面不会触发模型任务
- **全自动**：可不打开面板；面板用于钉住、待审、报错、迁移
- **任务透明**：记忆中心常驻显示运行、排队、失败和重试状态；暂停不会中断已经发出的请求
- **按聊隔离**：任务与失败状态写入当前聊天 metadata；切档不会把旧任务结果写入新档，刷新后可恢复
- **无硬刷新**：任务完成后只更新相关区域，不刷新酒馆页面
